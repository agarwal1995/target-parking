 ...
 * interface. The methods of the class should be use to handle the ParkingSlots
 * Business Logic in the diffrent {@value n} floor
 *
 */
public class ParkingServiceImpl implements ParkingService {
	...
	/*
	 * {@variable parkingFloorService} is the ParkingFloorService var that is
	 * being used to use the service for more designated methods
	 */

	/**
	 * code {@code placeVehicleToSlot} method should be used to assign the
	 * parking slot to a vehicle.This is a overridden method of the implemented
	 * interface ParkingService. If the vehicle is of the category
	 * {@value ELDER_PERSON_VEHICLE} or {@value ROYAL_CAR},then the search for
	 * an empty spot is being started from the ground floor itself which is the
	 * nearest. If the vehicle is of category {@value CAR} or {@value CARPOOL}
	 * then the search for an empty spot will be started from the 1st floor and
	 * then at last the ground floor. If the vehicle is of category
	 * {@value BIKE} then it will first check for any half filled bike spot
	 * ,then if not found a car spot will be assigned to the bike.
	 * 
	 */
	@Override
	public ParkingTicket allocateParkingToVehicle(Vehicle vehicle) throws Exception {
		...
		if (vehicle.getType() == VehicleCategory.CAR) {
			...
			if (vehicle.getVehicleProperty() == VehicleProperty.CARPOOL
					|| vehicle.getVehicleProperty() == VehicleProperty.DEFAULT) {
				ParkingLot.setNoOfCarParked(ParkingLot.getNoOfCarParked() + 1);
	...
	/**
	 * code {@code removeVehicleFromSlot} method should be used by the caller
	 * for the removal of the Vehicle from the ParkingSlot,and delete that
	 * particular ticket.
	 */
	@Override
	public void deallocateParkingFromVehicle(ParkingTicket parkingTicket) {

		parkingFloorService.deallocateParkingForVehicle(
				parkingLot.getParkingFloors().get(parkingTicket.getFloorIndex()), parkingTicket);

	}

	/**
	 * code {@code displaySummary} method should be used by the caller. for the
	 * quick summary of the Parking Lot Current Information.
	 */
	@Override
	...
	/**
	 * This code {@code isFull} method has been used to check whether there are
	 * any vacant spots present in the ParkingLot or they all have been
	 * filled.For Royal_Car objects it takes O(n) time complexity,which for
	 * other Object it provides the output in O(1) time complexity
	 * 
	 * @param vehicle
	 * @param parkingLot
	 * @return
