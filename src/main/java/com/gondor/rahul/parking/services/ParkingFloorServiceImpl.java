...
public class ParkingFloorServiceImpl implements ParkingFloorService {
	...
	/**
	 * This code {@code isFull} method is a overridden method of the 
	 * interface ParkingFloorService and has been used to check
	 * whether there are any vacant spots present in the current
	 * ParkingFloor Object or they all have been filled. It takes 
	 * constant time for all different types of Objects. 
	 *
	 */
	@Override
	public boolean isFull(ParkingFloor parkingFloor,Vehicle vehicle) {
		...
		if(vehicleCategory == VehicleCategory.BIKE) {
			return (parkingFloor.getFreeCarSpotsInCurrentFloor()==0 && parkingFloor.getFreeBikeSpotsInCurrentFloor() == 0);
		}
	...
	/**
	 * code {@code placeVehicleToSpot} method should be used to assign the parking slot to 
	 * a vehicle for a particular floor.This is a overridden method of the implemented interface ParkingFloorService.
	 * If the vehicle is of category {@value CAR} or {@value CARPOOL} or {@value ELDER_PERSON_VEHICLE} then it will
	 * search for the empty spots and will increments the count variable of the assigned vehicles & decrement the freespots
	 * accordingly. 
	 * If the vehicle is of the category {@value ROYAL_CAR},then it will use both the stack of the slots.
	 * If the vehicle is of category {@value BIKE} then for different stack it will have a fixed size for the capacity of the Slots
	 * For {@value Level.Lower} the capacity for a Bike Spot is 2 and for {@value Level.Upper} Stack the capacity 
	 * for the bike spot is 3.
	 * As The Size of the various vehicles type differ and have been defined beforehand ,therefore to check for particular vehicles
	 * slot they have been hardcoded.
	 * 
	 */
	@Override
	public ParkingTicket allocateParkingToVehicle(ParkingFloor parkingFloor, Vehicle vehicle) {
		...
		if(vehicle.getType() == VehicleCategory.ROYAL_CAR) {
			...
			parkingTicket  = allocateParkingToVehicleRoyalCar(parkingFloor,lowerSlots,upperSlots,parkingTicket,vehicle);
	...
	/**
	 * code {@code allocateParkingToVehicleCar} method is a method to allocate
	...
	private ParkingTicket allocateParkingToVehicleCar(ParkingFloor parkingFloor,ParkingSlot[] lowerSlots,
		...
		for(int i=0;i<20;i++){
			...
			if(lowerSlots[i].getFilled()==0) {
				...
				parkingTicket = new ParkingTicket(lowerSlots[i].getFloorIndex(),lowerSlots[i].getSpotNumber(),StackLevel.LOWER,vehicle);
				return parkingTicket;
			}
			...
			if(upperSlots[i].getFilled()==0) {
				...
				parkingTicket = new ParkingTicket(upperSlots[i].getFloorIndex(),upperSlots[i].getSpotNumber(),StackLevel.UPPER,vehicle);
				return parkingTicket;
			}
		}
		return parkingTicket;
	...
	/**
	 * code {@code allocateParkingToVehicleRoyalCar} method is a method to allocate
	 * a parking slot for the vehicle Car.
	 * 
	 * @param parkingFloor
	 * @param lowerSlots
		...
		if(parkingTicket==null) {
			...
			for(int i=0;i<20;i++) {
				if(lowerSlots[i].getFilled()==0) {
					...
					parkingTicket = new ParkingTicket(lowerSlots[i].getFloorIndex(),upperSlots[i].getSpotNumber(),StackLevel.UPPER,vehicle);
					return parkingTicket;
				}
			}
		}
	...
	/*
	 * (non-Javadoc)
	 * @see com.gondor.rahul.parking.services.ParkingFloorService#removeVehicleFromSpot(com.gondor.rahul.parking.models.parking.ParkingTicket, com.gondor.rahul.parking.models.parking.ParkingFloor)
	 * code {@code removeVehicleFromSpot} method should be used to remove the vehicle from the spot(i.e free the spot)
	 *  and decrease the assigned vehicle count and at the same time increase the free slots count.
	 *  If the Vehicle is if category {@value BIKE} then if the slot become empty after removing the vehicle,then the
	 *  slot becomes available again for the {@value CAR}
	 *  As The Size of the various vehicles type differ and have been defined beforehand ,therefore to check for particular vehicles
	 * slot they have been hardcoded.
	 * 
	 */
	...
	public void deallocateParkingForVehicle(ParkingFloor parkingFloor,ParkingTicket parkingTicket) {
		...
		if(vehicle.getType() == VehicleCategory.ROYAL_CAR) {
			...
			deallocateParkingForVehicleRoyalCar(parkingFloor,parkingTicket,vehicle,slotNumber,lowerSlots,upperSlots);
	...
	/**
	 * code {@code deallocateParkingForVehicleCar} is the method that should be used for the deallocating
	 * of the Car Vehicle from the ParkingSlot
	 * 
	 * @param parkingFloor
	 * @param parkingTicket
	 * @param vehicle
	...
	/**
	 * code {@code deallocateParkingForVehicleRoyalCar} is the method that should be used for the deallocating
	 * of the Royal Vehicle from the ParkingSlot
	 * 
	 * @param parkingFloor
	 * @param parkingTicket
	 * @param vehicle
	 ...
	 * @param upperSlots
	 */
	private void deallocateParkingForVehicleRoyalCar(ParkingFloor parkingFloor, ParkingTicket parkingTicket, Vehicle vehicle,
			int slotNumber, ParkingSlot[] lowerSlots, ParkingSlot[] upperSlots) {
		ParkingLot.setFreeCarSpots(ParkingLot.getFreeCarSpots()+2);
	...
	/**
	 * code {@code deallocateParkingForVehicleBike} is the method that should be used for the deallocating
	 * of the Bike Vehicle from the ParkingSlot
	 * 
	 * @param parkingFloor
	 * @param parkingTicket
	 * @param vehicle
	 ...
	 * @param upperSlots
	 */
	private void deallocateParkingForVehicleBike(ParkingFloor parkingFloor, ParkingTicket parkingTicket, Vehicle vehicle,
			int slotNumber, ParkingSlot[] lowerSlots, ParkingSlot[] upperSlots) {
